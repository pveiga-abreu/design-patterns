...
class Criador_Nota_Fiscal(object):
    ...
    def Constroi(self):
        ...
        if self.__observadores is None: self.__observadores = []

        return Nota_Fiscal(razao_social=self.__razao_social, cnpj=self.__cnpj,
                            itens=self.__itens, data_emissao=self.__data_emissao,
                            detalhes=self.__detalhes, observadores=self.__observadores)
